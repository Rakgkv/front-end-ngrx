import { fetchCountiresFailure, fetchCountiriesByRegion, fetchCountiriesByRegionFailure,
        fetchCountiriesByRegionSuccess, fetchCountries, fetchCountriesSuccess, fetchCountryDetail, 
        fetchCountryDetailFailure, fetchCountryDetailSuccess } from './country.actions';
...
describe('Countries API actions', () => {
    ...
    describe('Get Country Fetch Country detail Success action', () => {
        it('Should return the Fetch Countries region Success action', () => {
        expect(fetchCountryDetailSuccess.type).toBe('[Country] Fetch Country Details Success');
        });
    });

    describe('Get Country Fetch Country detail action', () => {
        it('Should return the Fetch Countries region failure action', () => {
        expect(fetchCountryDetailFailure.type).toBe('[Country] Fetch Country Details Failure');
        });
    });
});
